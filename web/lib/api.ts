...
export const api = {
  getStocks: () => request<Stock[]>(`${API_BASE_URL}/api/v1/stocks`),
  getPrices: (ticker: string, limit = 30) =>
    request<Price[]>(`${API_BASE_URL}/api/v1/prices/${ticker}?limit=${limit}`),
  getAllSignals: () => request<Signal[]>(`${API_BASE_URL}/api/v1/signals/all`),
  getSignal: (ticker: string) => request<Signal>(`${API_BASE_URL}/api/v1/signals/${ticker}`),
