...
export const api = {
  getStocks: () =>
    request<Stock[]>(`${API_BASE_URL}/api/v1/stocks`, {
      headers: authHeaders(),
    }),

  getPrices: (ticker: string, limit = 30) =>
    request<Price[]>(
      `${API_BASE_URL}/api/v1/prices/${ticker}?limit=${limit}`,
      {
        headers: authHeaders(),
      }
    ),
  ...
  getAllSignals: () =>
    request<Signal[]>(`${API_BASE_URL}/api/v1/signals/all`, {
      headers: authHeaders(),
    }),

  getSignal: (ticker: string) =>
    request<Signal>(`${API_BASE_URL}/api/v1/signals/${ticker}`, {
      headers: authHeaders(),
    }),

  explainSignal: (ticker: string) =>
    request<{ ticker: string; explanation: string }>(
      `${API_BASE_URL}/api/v1/signals/${ticker}/explain`,
      {
        headers: authHeaders(),
      }
    ),
  ...
  getMarketSnapshot: () =>
    request<MarketSnapshot>(
      `${MARKET_INTEL_BASE_URL}/market/snapshot`
    ),

  getNews: () =>
    request<{ articles: NewsArticle[] }>(
     '${MARKET_INTEL_BASE_URL}/market/news'
    ),
